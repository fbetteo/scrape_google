...
class Flight:
    ...
    def _classify_arg(self, arg):
        if ("AM" in arg or "PM" in arg) and len(self._times) < 2 and ":" in arg:
        ...
        elif (arg.endswith("CO2") or arg.endswith("CO2e")) and self._co2 is None:
            # co2
            ...
            ("hr" in arg and arg[-3:].isupper())
            or ("min" in arg and arg[-3:].isupper())
            or (len(arg.split(", ")) > 1 and arg.isupper())
        ):
            # 1 stop + time at stop
            # or multiple stops
            self._stops = arg
